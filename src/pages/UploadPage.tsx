...
import { Upload, X, Check, Copy, ExternalLink, Image as ImageIcon, AlertCircle, FileCheck, RefreshCw, Layers } from 'lucide-react';
import { imageApi } from '../lib/api';
import { UploadProgressFile, UploadResult } from '../types';
...
export const UploadPage: React.FC<UploadPageProps> = ({ navigate }) => {
  ...
  const startBatchUpload = async () => {
    ...
    for (const item of filesList) {
      ...
      try {
        const uploadRes = await imageApi.uploadFile(item.file, (percent) => {
          setFilesList((prev) =>
        ...
        setFilesList((prev) =>
          prev.map((f) =>
            f.id === item.id
              ? { ...f, status: 'completed', progress: 100, result: uploadRes }
              : f
          )
        );
      } catch (err: any) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-extrabold text-white">Resim Yükle</h1>
        <p className="text-slate-400 text-sm">
          Sürükleyip bırakın veya dosyalarınızı seçin. (JPG, PNG, WEBP, GIF - Maksimum 20 MB)
        </p>
      </div>
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">
                  Aynı anda birden fazla resim seçebilirsiniz
                </p>
              </div>
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                      ...
                      {item.status === 'uploading' && (
                        <div className="flex items-center gap-2 w-32">
                          ...
                          <span className="text-blue-400 font-mono text-[11px] font-bold">%{item.progress}</span>
                        </div>
                      )}
